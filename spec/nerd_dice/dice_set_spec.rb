...
RSpec.describe NerdDice::DiceSet do
  ...
  describe "highest and with_advantage methods" do
    ...
    let(:dice_4d6_with_3_1_4_1_rolled) { described_class.new 6, 4, randomization_technique: :random_rand }
    let(:dice_2d20_with_17_19_rolled) { described_class.new 20, 2, randomization_technique: :random_rand }
    let(:dice_d100_with_55_rolled) { described_class.new 100, randomization_technique: :random_rand }

    it "returns self" do
      expect(dice_4d6_with_3_1_4_1_rolled.highest(2)).to be_a(described_class)
    end

    it "eliminates only the lowest die if no argument" do
      # a 17 and a 19
      expect(dice_2d20_with_17_19_rolled.highest.total).to eq(19)
    end

    it "returns the highest n of the dice" do
      # 3, 1, 4, 1
      expect(dice_4d6_with_3_1_4_1_rolled.highest(3).total).to eq(8)
    end

    it "aliases highest as with_advantage" do
      # 3, 1, 4, 1
      expect(dice_4d6_with_3_1_4_1_rolled.with_advantage(3).total).to eq(8)
    end
    ...
    it "raises error if the argument exceeds the number_of_dice" do
      expect { dice_4d6_with_3_1_4_1_rolled.highest(5) }.to raise_error(
  ...
  describe "lowest and with_disadvantage methods" do
    ...
    it "returns self" do
      expect(dice_4d6_with_3_1_4_1_rolled.lowest(2)).to be_a(described_class)
    end

    it "eliminates only the highest die if no argument" do
      # a 17 and a 19
      expect(dice_2d20_with_17_19_rolled.lowest.total).to eq(17)
    end

    it "returns the lowest n of the dice" do
      # 3, 1, 4, 1
      expect(dice_4d6_with_3_1_4_1_rolled.lowest(3).total).to eq(5)
    end

    it "aliases lowest as with_disadvantage" do
      # 3, 1, 4, 1
      expect(dice_4d6_with_3_1_4_1_rolled.with_disadvantage(3).total).to eq(5)
    end

    it "returns all dice if the argument matches number_of_dice" do
      expect(dice_d100_with_55_rolled.lowest.total).to eq(dice_d100_with_55_rolled.total)
    end

    it "raises error if the argument exceeds the number_of_dice" do
      expect { dice_4d6_with_3_1_4_1_rolled.lowest(5) }.to raise_error(
        ArgumentError, "Argument cannot exceed number of dice"
      )
    end
  end
