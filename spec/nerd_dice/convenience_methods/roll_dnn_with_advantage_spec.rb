...
RSpec.describe NerdDice::ConvenienceMethods, ".roll_dnn_with_advantage" do
  ...
  describe "roll_dNN_with_advantage method" do
    ...
    it "defines the method after it is called" do
      magic.roll_d100_with_advantage(**method_options)
      expect(magic.public_methods).to include(:roll_d100_with_advantage)
    end

    it "handles subsequent calls without calling method_missing" do
      expect(magic).to receive(:method_missing).with(:roll_d12_with_advantage, **method_options).once.and_call_original
      magic.roll_d12_with_advantage(**method_options)
      magic.roll_d12_with_advantage
      magic.roll_d12_with_advantage(**method_options)
    end

    it "responds to methods matching the pattern" do
      expect(magic.respond_to?(:roll_d6_with_advantage)).to be(true)
    end
  end
  ...
  describe "roll_dNN_with_advantage with bonuses and penalties" do
    it "calls calls NerdDice.roll_dice with correct arguments, keywords with bonus" do
      ...
      magic.roll_d20_with_advantage_plus6(**method_options)
    end
    ...
    it "calls calls NerdDice.roll_dice with correct arguments, keywords with penalty" do
      ...
      expect(NerdDice).to receive(:roll_dice).with(20, 2, **merged_options).and_call_original
      magic.roll_d20_with_advantage_m5(**method_options)
    end
    ...
    it "responds to methods matching the pattern" do
      expect(magic.respond_to?(:roll_d20_with_advantage_minus4)).to be(true)
    end
  end
end
