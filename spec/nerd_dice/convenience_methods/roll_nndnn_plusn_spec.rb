...
# roll_(N)dNN_plusN pattern spec
# Covers situation pattern of /roll_\d*d\d+_plus_\d+/
...
# * Examples
#   * roll_3d20_plus5 roll 3 d20 add 5
#   * roll_d8_plus5 => roll 1 d8 add 5
RSpec.describe NerdDice::ConvenienceMethods, ".roll_nndnn_plusn" do
  ...
  describe "roll_NdNN_plusN method" do
    it "calls NerdDice.roll_dice with correct arguments and keywords" do
      ...
      magic.roll_3d20_plus6(**method_options)
    end

    it "calls NerdDice.roll_dice with no keywords" do
      expect(NerdDice).to receive(:roll_dice).with(8, 22, bonus: 6).and_call_original
      magic.roll_22d8_plus6
    end

    it "works if you specify 1dNN" do
      expect(NerdDice).to receive(:roll_dice).with(4, 1, **merged_options).and_call_original
      magic.roll_1d4_plus6(**method_options)
    end

    it "defines the method after it is called" do
      magic.roll_6d100_plus6(**method_options)
      expect(magic.public_methods).to include(:roll_6d100_plus6)
    end

    it "handles subsequent calls without calling method_missing" do
      expect(magic).to receive(:method_missing).with(:roll_2d12_plus6, **method_options).once.and_call_original
      magic.roll_2d12_plus6(**method_options)
      magic.roll_2d12_plus6
      magic.roll_2d12_plus6(**method_options)
    end
    ...
    it "raises error if bonus is inconsistent with kwargs" do
      expect { magic.roll_2d12_plus6 bonus: 5 }.to raise_error(
  ...
  describe "roll_dNN_plusN method" do
    ...
    it "works if you specify 1dNN" do
      expect(NerdDice).to receive(:roll_dice).with(4, 1, **merged_options).and_call_original
      magic.roll_d4_plus6(**method_options)
    end

    it "defines the method after it is called" do
      magic.roll_d100_plus6(**method_options)
      expect(magic.public_methods).to include(:roll_d100_plus6)
    end

    it "handles subsequent calls without calling method_missing" do
      expect(magic).to receive(:method_missing).with(:roll_d12_plus6, **method_options).once.and_call_original
      magic.roll_d12_plus6(**method_options)
      magic.roll_d12_plus6
      magic.roll_d12_plus6(**method_options)
    end
    ...
    it "raises error if bonus is inconsistent with kwargs" do
      expect { magic.roll_d12_plus6 bonus: 5 }.to raise_error(
        NerdDice::Error, /#{get_bonus_error_message(5, 6)}/
      )
    end
  end
